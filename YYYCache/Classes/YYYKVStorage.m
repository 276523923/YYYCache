...
static NSString *const kDBFileName = @"yyymanifest.sqlite";
static NSString *const kDBShmFileName = @"yyymanifest.sqlite-shm";
static NSString *const kDBWalFileName = @"yyymanifest.sqlite-wal";
static NSString *const kDataDirectoryName = @"yyydata";
static NSString *const kTrashDirectoryName = @"yyytrash";

/*
 File:
 /path/
 /yyymanifest.sqlite
 /yyymanifest.sqlite-shm
 /yyymanifest.sqlite-wal
 /data/
 ...
 SQL:
 create table if not exists yyymanifest (
 key                 text,
 filename            text,
 size                integer,
 inline_data         blob,
 ...
 create index if not exists last_access_time_idx on yyymanifest(last_access_time);
 */

/// Returns nil in App Extension.
static UIApplication *_YYYSharedApplication() {
...
@implementation YYYKVStorageItem
@end

@implementation YYYKVStorage {
...
- (BOOL)_dbUpdateAccessTimeWithKey:(NSString *)key {
    NSString *sql = @"update yyymanifest set last_access_time = ?1 where key = ?2;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt)
        return NO;
...
- (BOOL)_dbUpdateAccessTimeWithKeys:(NSArray *)keys {
    if (![self _dbCheck])
        return NO;
    int t = (int) time(NULL);
    NSString *sql = [NSString stringWithFormat:@"update yyymanifest set last_access_time = %d where key in (%@);", t, [self _dbJoinedKeys:keys]];
...
- (BOOL)_dbDeleteItemWithKey:(NSString *)key {
    NSString *sql = @"delete from yyymanifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt)
        return NO;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
...
- (BOOL)_dbDeleteItemWithKeys:(NSArray *)keys {
    if (![self _dbCheck])
        return NO;
    NSString *sql = [NSString stringWithFormat:@"delete from yyymanifest where key in (%@);", [self _dbJoinedKeys:keys]];
    sqlite3_stmt *stmt = NULL;
    int result = sqlite3_prepare_v2(_db, sql.UTF8String, -1, &stmt, NULL);
    if (result != SQLITE_OK) {
        if (_errorLogsEnabled)
...
- (BOOL)_dbDeleteItemsWithSizeLargerThan:(int)size {
    NSString *sql = @"delete from yyymanifest where size > ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt)
        return NO;
...
- (YYYKVStorageItem *)_dbGetItemFromStmt:(sqlite3_stmt *)stmt excludeInlineData:(BOOL)excludeInlineData {
    ...
    int expiration_time = sqlite3_column_int(stmt, i++);

    YYYKVStorageItem *item = [YYYKVStorageItem new];
    if (key)
...
- (YYYKVStorageItem *)_dbGetItemWithKey:(NSString *)key excludeInlineData:(BOOL)excludeInlineData {
    NSString *sql = excludeInlineData ? @"select key, filename, size, modification_time, last_access_time, extended_data from yyymanifest where key = ?1;" : @"select key, filename, size, inline_data, modification_time, last_access_time, extended_data ,expiration_time from yyymanifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt)
        return nil;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
...
- (NSData *)_dbGetValueWithKey:(NSString *)key {
    NSString *sql = @"select inline_data from yyymanifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt)
        return nil;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
...
- (NSString *)_dbGetFilenameWithKey:(NSString *)key {
    NSString *sql = @"select filename from yyymanifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt)
        return nil;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
...
- (NSMutableArray *)_dbGetAllExpirationTime {
    ...
    if (!stmt)
        return nil;
    NSMutableArray *expiratironTimes = [NSMutableArray new];
    do {
        int result = sqlite3_step(stmt);
        if (result == SQLITE_ROW) {
            ...
            if (_errorLogsEnabled)
                NSLog(@"%s line:%d sqlite query error (%d): %s", __FUNCTION__, __LINE__, result, sqlite3_errmsg(_db));
            expiratironTimes = nil;
            break;
        }
    } while (1);
    return expiratironTimes;
}

- (NSMutableArray *)_dbGetFilenameWithKeys:(NSArray *)keys {
    if (![self _dbCheck])
        return nil;
    NSString *sql = [NSString stringWithFormat:@"select filename from yyymanifest where key in (%@);", [self _dbJoinedKeys:keys]];
    sqlite3_stmt *stmt = NULL;
    int result = sqlite3_prepare_v2(_db, sql.UTF8String, -1, &stmt, NULL);
    if (result != SQLITE_OK) {
        if (_errorLogsEnabled)
...
- (NSMutableArray *)_dbGetFilenamesWithSizeLargerThan:(int)size {
    NSString *sql = @"select filename from yyymanifest where size > ?1 and filename is not null;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt)
        return nil;
    sqlite3_bind_int(stmt, 1, size);
...
- (NSMutableArray *)_dbGetItemSizeInfoOrderByTimeAscWithLimit:(int)count {
    NSString *sql = @"select key, filename, size from yyymanifest order by last_access_time asc limit ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt)
        return nil;
...
- (int)_dbGetItemCountWithKey:(NSString *)key {
    NSString *sql = @"select count(key) from yyymanifest where key = ?1 and (expiration_time > ?2 or expiration_time = 0);";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt)
        return -1;
    int currTime = (int) time(NULL);
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
    sqlite3_bind_int(stmt, 2, currTime);
    int result = sqlite3_step(stmt);
    if (result != SQLITE_ROW) {
        if (_errorLogsEnabled)
            NSLog(@"%s line:%d sqlite query error (%d): %s", __FUNCTION__, __LINE__, result, sqlite3_errmsg(_db));
...
- (int)_dbGetTotalItemSize {
    NSString *sql = @"select sum(size) from yyymanifest;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt)
        return -1;
    int result = sqlite3_step(stmt);
...
- (int)_dbGetTotalItemCount {
    NSString *sql = @"select count(*) from yyymanifest;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt)
        return -1;
    int result = sqlite3_step(stmt);
...
- (instancetype)init {
    ...
    return [self initWithPath:@"" type:YYYKVStorageTypeFile];
}

- (instancetype)initWithPath:(NSString *)path type:(YYYKVStorageType)type {
    if (path.length == 0 || path.length > kPathLengthMax) {
        NSLog(@"YYKVStorage init error: invalid path: [%@].", path);
        return nil;
    }
    if (type > YYYKVStorageTypeMixed) {
    ...
    _trashQueue = dispatch_queue_create("com.ibireme.cache.disk.yyytrash", DISPATCH_QUEUE_SERIAL);
    _queue = dispatch_queue_create("com.ibireme.cache.disk.yyystorage", DISPATCH_QUEUE_CONCURRENT);
...
- (void)dealloc {
    UIBackgroundTaskIdentifier taskID = [_YYYSharedApplication() beginBackgroundTaskWithExpirationHandler:^{
    }];
    [self _dbClose];
    if (taskID != UIBackgroundTaskInvalid) {
        [_YYYSharedApplication() endBackgroundTask:taskID];
    }
}

- (BOOL)saveItem:(YYYKVStorageItem *)item {
...
- (BOOL)saveItemWithKey:(NSString *)key value:(NSData *)value filename:(NSString *)filename expirationTime:(time_t)expirationTime extendedData:(NSData *)extendedData {
    ...
    if (filename.length) {
        ...
        if (![self _dbSaveWithKey:key value:value fileName:filename extendedData:extendedData expirationTime:expirationTime]) {
            [self _fileDeleteWithName:filename];
            return NO;
        }
        return YES;
    } else {
        if (_type != YYYKVStorageTypeSQLite) {
            NSString *filename = [self _dbGetFilenameWithKey:key];
            if (filename) {
                [self _fileDeleteWithName:filename];
            }
        }
        return [self _dbSaveWithKey:key value:value fileName:nil extendedData:extendedData expirationTime:expirationTime];
    }
}
...
- (BOOL)removeItemForKey:(NSString *)key {
    if (key.length == 0)
        return NO;
    switch (_type) {
        case YYYKVStorageTypeSQLite: {
            return [self _dbDeleteItemWithKey:key];
        }
            break;
        case YYYKVStorageTypeFile:
        case YYYKVStorageTypeMixed: {
            NSString *filename = [self _dbGetFilenameWithKey:key];
            if (filename) {
                [self _fileDeleteWithName:filename];
            }
...
- (BOOL)removeItemsToFitSize:(int)maxSize {
    ...
    do {
        ...
        for (YYYKVStorageItem *item in items) {
            if (total > maxSize) {
                if (item.filename) {
...
- (YYYKVStorageItem *)getItemForKey:(NSString *)key {
    if (key.length == 0)
        return nil;
...
- (YYYKVStorageItem *)getItemInfoForKey:(NSString *)key {
    if (key.length == 0)
        return nil;
    YYYKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:YES];
    return item;
}
...
- (int)getItemsCount {
    Lock();
    int count = [self _dbGetTotalItemCount];
    Unlock();
    return count;
}
...
- (int)getItemsSize {
    Lock();
    int size = [self _dbGetTotalItemSize];
    Unlock();
    return size;
}
